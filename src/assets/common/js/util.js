...
class util {
	...
	constructor() {
		...
		this.isApp	= $.cookie('BD-EGO_TERMINAL')?true:false
		...
		return $.ajax({
			...
			success: function(data, textStatus, request) {
				...
				if (typeof(data) == 'string') {
					...
					if (json.responseDate) {
						datas.systime = new Date(request.getResponseHeader('date')).getTime()
					}
					return This.error(datas, json);
				} else {
					if (json.responseDate) {
						data.systime = new Date(request.getResponseHeader('date')).getTime()
					}
					return This.error(data, json);
				}
			},
	...
	*/
	...
	formatSeconds(value, type) {
		...
		var result = this.getZero(minute) + fen + this.getZero(parseInt(second)) + miao;
		if (minute > 0) {
			result = this.getZero(parseInt(minute)) + fen + this.getZero(parseInt(second)) + miao;
		}
		if (hour > 0) {
			result = this.getZero(parseInt(hour)) + shi + this.getZero(parseInt(minute)) + fen + this.getZero(parseInt(second));
		}
		return result;
	}
    ...
    */
	scrollGetData(json) {
		if ($('div.container').length) {
			if (!$('#bottomscolltop').length) {
				$('div.container').append('<div id="bottomscolltop"></div>')
			}
		} else {
			if (!$('#bottomscolltop').length) {
				$('body').append('<div id="bottomscolltop"></div>')
			}
		}

		let lastObj = $('#bottomscolltop');
		$(window).on('scroll', function() {
			let scrollbottom = (lastObj.offset().top - $(window).height()) - $(window).scrollTop()
			let conbottom = json.bottomTop || config.bottomTop
			
			if ( scrollbottom < conbottom) {
				if(json.scrollbegin&&json.scrollbegin.toString()){
					if (json.scrollbegin) {
						json.scrollbegin = false;
						json.callback();
					}
				}else{
	...
	handleAppVersion(){
		let version = ''
		let cookies = $.cookie('BD-EGO_TERMINAL').toLowerCase()
		try{
			if(cookies.indexOf('ios') != -1){
				version = parseInt(cookies.split(',')[1].replace(/\./g,''));
			}else if(cookies.indexOf('android') != -1){
				version = parseInt(cookies.split('build')[0].split(',')[1].replace(/\./g,''))
			}
		}catch(err){
			version = 271
		}
